...
class ENVIRONMENT : public RaisimGymEnv {
 ...
 public:
  ...
  explicit ENVIRONMENT(const std::string& resourceDir, const Yaml::Node& cfg, bool visualizable) :
    ...
    /// add objects
    go1_ = world_->addArticulatedSystem(resourceDir_+"/go1/urdf/go1_v2.urdf");
    go1_->setName("go1");
    go1_->setControlMode(raisim::ControlMode::FORCE_AND_TORQUE);
    ...
    if (nJoints_ != 12) {
      throw std::invalid_argument("number of joints should equal 12");
    }
    ...
    // Vector12d curr_ref_joint_torque;
    ...
    footIndices_.insert(go1_->getBodyIdx("FL_calf"));
    footIndices_.insert(go1_->getBodyIdx("FR_calf"));
    footIndices_.insert(go1_->getBodyIdx("RL_calf"));
    footIndices_.insert(go1_->getBodyIdx("RR_calf"));
  ...
  void reset() final {
    ...
    go1_->getCollisionBody("FL_foot/0").setMaterial("foot_material");
    go1_->getCollisionBody("FR_foot/0").setMaterial("foot_material");
    go1_->getCollisionBody("RL_foot/0").setMaterial("foot_material");
    go1_->getCollisionBody("RR_foot/0").setMaterial("foot_material");
    world_->setMaterialPairProp("ground_material", "foot_material",
                                frictionCoeff, restitutionCoeff,
                                restitutionThresh);

    // randomized inertial parameters
    std::vector<double> &mass = go1_->getMass();
    ...
    for (int joint_idx = 0; joint_idx < 12; joint_idx++) {
  ...
  float step(const Eigen::Ref<EigenVec>& action) final {
    ...
    for(int i=0; i< int(control_dt_ / simulation_dt_ + 1e-10); i++){
      ...
      go1_->setGeneralizedForce(torqueCommand_);
      ...
      // log_vec.segment(21, 8) = go1_->getGeneralizedForce().e().tail(nJoints_); // simulated joint torques
    ...
    // go1_->setState(reference, gv_init_);
  ...
  void updateObservation() {
    ...
    sensor_reading_ <<
        imu_reading.e(),
        gc_.tail(12) + joint_offset_, /// joint angles, with randomized joint offset
 ...
 private:
  ...
  Vector12d ref_joint_vel_;
  Vector12d ref_joint_torque_;
  Vector12d offset_joints_;
